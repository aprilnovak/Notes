...
\beq
\label{eq:MB}
M(V,T)=\left(\frac{m}{2\pi kT}\right)^{3/2}e^{-mV^2/2kT}
\eeq
...
\beqa
\frac{\partial M(V_{\text{most probable}},T)}{\partial V}&=0\\
V_{\text{most probable}}&=\sqrt{\frac{2kT}{m}}
\eeqa

And the expectation value of the velocity is:

\beqa
\langle V\rangle\equiv&\frac{\int_{0}^\infty dV VM(V,T)}{\int_{0}^\infty dV M(V,T)}\\
=&\sqrt{\frac{8kT}{m\pi}}
\eeqa
