...
In Cartesian coordinates, a neutron moves along constant \(\hO\) until suffering a collision. However, in curvilinear coordinates (cylindrical and spherical), as a neutron streams, the angular coordinates are constantly changing.
...
where \(C_j\) is the delayed neutron precursor concentration of the \(j\)-th kind that {\it always} decays by \(\beta\) decay, and is hence some fraction of the actual precursor concentration; and \(\lambda\) is the delayed neutron precursor decay constant. Similar to prompt fission neutrons, delayed neutrons are frequently approximated as being born isotropically, and angle integration of the delayed neutron energy and angled spectrum yields only an energy dependence:
...
\beqa
\label{eq:streaming2}
\left\lbrack\int dS n\seat v(E)\hO\cdot\hat{n}\right\rbrack dEd\hO\equiv&\left\{\int d\volume\nabla\cdot\left\lbrack n\seat v(E)\hO\right\rbrack\right\} dEd\hO\\
...
\frac{\partial C_j(\vv{r},t)}{\partial t}=\beta_j\int_{0}^\infty dE'\int_{4\pi}\hO'\nu(E')\Sigma_f(E')\psi(\vv{r},E',\hO',t)-\lambda_jC_j(\vv{r},t)\hspace{1cm}\text{for \(j=1,\cdots,J\)}
\eeq
...
%When considering multiple neutron energies, Eq. \eqref{eq:k1} can be made more detailed to reflect neutron movement between energy groups by scattering and fission. 
...
\beqa
\label{eq:lambda_n}
\lambda_n=&\ vD\left\lbrack \left(\frac{n\pi}{\tilde{a}}\right)^2+\frac{\Sigma_a-\nu\Sigma_f}{D}\right\rbrack\\
=&\ v\Sigma_a\left(1+L^2B_n^2\right)\left(1-\frac{k_\infty}{1+L^2B_n^2}\right)
\eeqa
...
\Sigma_{i,g}(\vv{r},\hO,t)\equiv\frac{1}{\psi_g\sat}\int_{E_{g-1}}^{E_g}dE\Sigma_{i}\seat\psi\seat\\
\eeqa
...
\psi_g(\vv{r},\hO,t)=\psi(\vv{r}, \hO,t)\int_{E_{g-1}}^{E_g}dEf(E)
\eeq

\beqa
\label{eq:GroupwiseQuantityProduct2}
\Sigma_{i,g}(\vv{r},\hO,t)=\int_{E_{g-1}}^{E_g}dE\Sigma_{i}\seat f(E)
\eeqa
...
&\hspace{3cm}\sum_{j=1}^J\chi_{d,j,g}(\hO)\lambda_jC_j(\vv{r},t)+ S_g\sat
\eeqa
